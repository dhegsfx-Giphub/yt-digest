...
def fetch_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        if text.strip():
            log.info(f"Got YouTube captions for {video_id}")
            return text
    except Exception:
        pass
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if text.strip():
                log.info(f"Supadata transcribed {video_id} successfully")
                return text
        log.warning(f"Supadata returned {resp.status_code} for {video_id}")
        return None
    except Exception as e:
